...
dim(covariates_geno_ctpcs_tech_cc) # 125 146
...
```{r model_matrix}
# create model matrix
mod <- model.matrix(~ no2_y1_2019v +
                      sex + maternal_smoking_y1 + maternal_education_length + pss_6m + 
...
mod0 <- model.matrix(~ sex + maternal_smoking_y1 + maternal_education_length + 
                       genotyping.PC1 + genotyping.PC2 + genotyping.PC3 + pss_6m + 
                       Comp.1 + Comp.2 + Comp.3 + Comp.4 + Comp.5 + Comp.6, 
                     data=covariates_geno_ctpcs_tech_cc)
...
save(sv_y1, file = here("output_data", 
                        "linear_regression", 
                        "2022-09-27_postnatal_svs.RData"))
...
ggsave(plot = pn_sv_scree, 
       filename = here("figures", 
                       "linear_regression", 
                       "postnatal", 
                       "2022-09-28_pn_sv_heat_scree.png"),
...
colnames(covariates_geno_ctpcs_tech_cc)[147:160] <-
  paste("sv",colnames(covariates_geno_ctpcs_tech_cc)[147:160], sep="")
...
corr <- corr.mat(covariates_geno_ctpcs_tech_cc %>% 
                   dplyr::select(no2_y1_entire, sex, maternal_smoking_y1, 
                                 maternal_education_length, pss_6m,
                                 genotyping.PC1, genotyping.PC2, genotyping.PC3,
...
# t stats in bacon
set.seed(1234)
bc <- bacon(diff_ebayes$t[,"no2_y1_2019v"])

# get bacon pvals
diff_pvals_tech$pval_no2_y1_2019v_bacon <- pval(bc)

# bh adjusted bacon pvals
diff_pvals_tech$pval_no2_y1_2019v_bacon_adj  <-
  p.adjust(diff_pvals_tech$pval_no2_y1_2019v_bacon , method="BH")
...
pvector <- diff_pvals_tech$pval_no2_y1_2019v_bacon
...
diff_pvals_tech$db <- diff_pvals_tech$coeff_no2_y1_2019v * range
```
...
volcano_plot <- ggplot(diff_pvals_tech, 
  ...
  geom_point(data=subset(diff_pvals_tech, 
                         (pval_no2_y1_2019v_adj < 0.25 & db > 0.025) | 
                           (pval_no2_y1_2019v_adj < 0.25 & db < -0.025)), 
