...
volcano_plot <- ggplot(diff_pvals_tech, 
  ...
  geom_point(alpha=0.9, col = "#254A90", size=1) +
...
yearone_goi_df_pval <- yearone_goi_df  %>% arrange(p.value.no2_y1_2019v)
...
write.csv(yearone_goi_df_pval_db, 
          file=here("tables", 
                    "yearone",
                    "2022-10-07_goi_postnatal_cpgs.csv"))

# write table of top ten 
write.csv(yearone_goi_df_pval_db %>% 
            subset(effect_size > 0.025 | effect_size < -0.025) %>% 
            subset(-log10(p.value.no2_y1_2019v) > 1.6),
          here("tables", 
               "yearone", 
               "2022-10-07_top10goi_yearone_cpgs.csv"))
...
write.csv(goi_df_pval_db_sig, 
          here("tables", 
               "yearone", 
               "2022-10-07_toptable_yearone_goi.csv"))
...
write.csv(combp_anno_df_es_sub,
          here("output_data",
               "linear_regression",
               "2022-10-07_postnatalspecific_no2_combp_annotation_sigDMRs.csv"))
```
...
dmr1_atopy_vis <- 
  ...
  ggbeeswarm::geom_beeswarm(dodge.width=0.9, cex = 1.1, size = 1) +
  theme_bw() +
...
ggsave(plot = dmr1_atopy_vis,
       filename  = here("figures",
                        "linear_regression",
                        "postnatal",
                        "2022-11-11_no2_pn_dmr1_atopy_legend.png"), 
...
methmean_plot <- ggplot(covariates_geno_ctpcs_tech_cc,
  ...
  geom_ribbon(data = methmean_no2_me, 
              aes(x = no2_y1_2019v, 
                  ymin = conf.low, ymax = conf.high), 
              alpha = .2) + 
  geom_point(alpha=0.8, col = "#92C5EB", size=1) + 
  theme_bw() +
  theme(panel.grid = element_blank(),
        axis.text = element_text(size=9),
        panel.border = element_blank(),
...
# effect size
summary(meanmeth_lm)$coefficients["no2_y1_2019v", "Estimate"]*range_no2 # -0.0002507753
```
...
ggsave(plot = line_dnam_plot, 
       filename = here("figures", 
                       "linear_regression", 
                       "postnatal",
                       "2022-10-07_postnatal_line_dnam.png"),
       width = 4.0, height = 3.2, units = "cm")
...
# effect size
summary(line_meth_lm)$coefficients["no2_y1_2019v", "Estimate"]*range_no2 # -0.0004671128
```
