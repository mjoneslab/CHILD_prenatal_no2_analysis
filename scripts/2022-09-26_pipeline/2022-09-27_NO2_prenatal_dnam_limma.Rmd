...
ggsave(plot = geno_pc1_pc2, 
       filename="2022-11-17_CHILD_genopcs_race_ancestry_PC1_PC2.png",
       device = "png",
...
ggsave(plot = geno_pc1_pc3, 
       filename="2022-11-17_CHILD_genopcs_race_ancestry_PC1_PC3.png",
       device = "png",
       path =  here("figures", "genotyping"),
       width = 6,
       height = 4,
...
dmr_persistence <- lmer_pvals_dmrs %>% 
  ...
  select(coef_no2, coef_no2_age_one, cpg, sig, start, change) %>%
  ...
  facet_wrap(~start) + 
  scale_shape_manual(values = c(17,16), labels = c("Higher", "Lower"), name = "Change in coefficient\nmagnitude") + 
  scale_x_discrete(labels = c("Birth", "Age one"), name = element_blank()) +
  scale_y_continuous(name = "Prenatal NO2 coefficient (B)") +
  theme_bw() +
  theme(panel.grid.major.x = element_blank(),
        axis.text.x = element_text(size=9),
        axis.text.y = element_text(size=9))
...
ggsave(plot = dmr_persistence,
       filename = here("figures",
                   "linear_regression",
                   "prenatal",
                   "2022-11-17_dmr_persistence_legend.png"),
...
top100_persistence <- lmer_pvals_cpgs_results %>% 
  ...
  facet_wrap(~start) + 
  theme_bw() +
  scale_shape_manual(values = c(17,16), labels = c("Higher", "Lower")) + 
  scale_x_discrete(labels = c("Birth", "Age one"), name = element_blank()) +
  scale_y_continuous(name = "Prenatal NO2 coefficient (B)") +
...
ggsave(plot = top100_persistence,
       filename = here("figures",
                   "linear_regression",
                   "prenatal",
                   "2022-11-17_top100_persistence_length.png"),
       height = 7, width = 8, units="cm")
```
