...
ggsave(plot = studysite_prenatal_no2,
       filename = here("figures",
                       "linear_regression",
                       "prenatal",
                       "2023-02-27_studysite_vs_prenatalno2.png"),
...
ggsave(plot = studysite_prenatal_no2_persistence,
       filename = here("figures",
                       "linear_regression",
                       "prenatal",
                       "2023-02-27_studysite_prenatal_no2_persistence.png"),
       height = 9, width = 11, units="cm")
...
t.test(covariates_geno_ctpc_tech_cc %>% 
         dplyr::select(no2_prenatal_tm1_2019v, no2_y1t1_2019v, sample_id) %>% 
         na.omit() %>%
         pull(no2_prenatal_tm1_2019v),
       covariates_geno_ctpc_tech_cc %>% 
         dplyr::select(no2_prenatal_tm1_2019v, no2_y1t1_2019v, sample_id) %>% 
         na.omit() %>%
         pull(no2_y1t1_2019v), paired = T)

prenatal_vs_yearone_no2 <- ggplot(covariates_geno_ctpc_tech_cc %>% 
         dplyr::select(no2_prenatal_tm1_2019v, no2_y1t1_2019v, sample_id, studycentre) %>% 
         na.omit() %>% 
         mutate(higher = ifelse(no2_y1t1_2019v > no2_prenatal_tm1_2019v, 1, 0)) %>%
         pivot_longer(-c(sample_id, higher, studycentre)),
       aes(x=name, y = value, color = as.factor(higher))) +
  geom_point(alpha = 0.5) +
  geom_line(aes(group = sample_id), alpha = 0.8) +
  ...
  geom_signif(
    comparisons = list(c("no2_prenatal_tm1_2019v", "no2_y1t1_2019v")),
    annotations = c("1.8e-05"), 
    textsize = 3, color = "black", margin_top = 0.1
  ) + ylim(0, 35)
...
ggsave(plot = prenatal_vs_yearone_no2,
       filename = here("figures",
                       "linear_regression",
                       "prenatal",
                       "2023-02-28_prenatal_vs_yearone_no2_exp.png"),
