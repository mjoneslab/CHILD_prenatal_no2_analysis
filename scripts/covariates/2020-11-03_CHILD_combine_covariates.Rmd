...
f = list.files(path = "R:/Jones/Projects/CHILD/Data request Aug 2020/CP139 data files 2", 
               pattern = "*.csv",
               full.names = TRUE)
...
names(covariate_files) = str_extract(list.files(path = "R:/Jones/Projects/CHILD/Data request Aug 2020/CP139 data files 2", pattern = "*.csv"), '.*(?=\\.csv)')
...
g = list.files(path = "R:/Jones/Projects/CHILD/Data request Aug 2020/DR2", 
               pattern = "*.csv",
               full.names = TRUE)
...
#set names of list equal to the file names, without the file type
names(covariate_files2) = str_extract(list.files(path = "R:/Jones/Projects/CHILD/Data request Aug 2020/DR2", pattern = "*.csv"), '.*(?=\\.csv)')
...
h = list.files(path = "R:/Jones/Projects/CHILD/Data request Aug 2020/DR2", 
               pattern = "NO2*",
               full.names = TRUE)
...
names(covariate_files3) = sub(" ", "_", str_extract(list.files(path = "R:/Jones/Projects/CHILD/Data request Aug 2020/DR2", pattern = "NO2"), '.*(?=\\.xlsx)'))

#read out each list item into global environment as dataframe
list2env(covariate_files3, .GlobalEnv)

pm2.5 <- read.csv("R:/Jones/Projects/CHILD/PM_data/CHILD PM2.5 data.csv")
...
```{r check_order}
#read in order sheet
participants = as.data.frame(read_xlsx("R:/Jones/Projects/CHILD/Data request Aug 2020/CP139 data files 2/IDs to share DR139.xlsx"))
...
covariates <- select(covariates, -contains("subjectnumber", ignore.case = TRUE))
```
...
studycentre <-  select(covariates, matches("studycenter", ignore.case = TRUE))
...
covariates2 <- select(covariates, -matches("status|study|eligibility|subjecttype", ignore.case = TRUE)) #this is info about study site and group
...
save(covariates, file=here("output_data", "covariates", "2020-11-03_CHILD_all_covariates.Rdata"))
```