...
svobj = sva(cb_betas_geno_cc_clean, mod, mod0) # 15 factors
...
png(file=paste0('SV_ScreePlot.png'), width=1200, height=800)

par(mar=c(5,5,2,2))

barplot(vars[xp]*100, names.arg = names(vars[xp]), lwd=2, cex.lab=2, cex.axis=2, cex.names=1.6, ylim=c(0,5), col='black', ylab = 'Percentage of Variance', xlab='Surrogate Variables')
...
colnames(covariates_geno_ctpc_tech_cc)[133:147] <- 
...
meta_categorical <- covariates_geno_ctpc_tech_cc[,c("sex", "run", "chip",
                                                    "prenatal_maternal_smoking")] 
meta_continuous <- covariates_geno_ctpc_tech_cc[,c("no2_preg_entire", 
                                                   "genotyping.PC1",
                                                   "genotyping.PC2",
                                                   "genotyping.PC3",
                                                   "cbmc.PC1", "cbmc.PC2",
...
colnames(cb_pvals_tech) <- c(paste("pval", colnames(cb_pvals_tech)[1:30], sep = "_"), 
                         paste("coeff", colnames(cb_pvals_tech)[31:60], sep = "_"),
...
meanmeth_lm <- 
  lm(methmeans ~ no2_preg_entire + sex + gestational_days +
       ...
       cbmc.PC4 + cbmc.PC5 + cbmc.PC6 + 
       genotyping.PC1 + genotyping.PC2 + 
       genotyping.PC3 + run + row + sv1 + sv2 + sv3 + sv4 + sv5 + sv6, 
     data=covariates_geno_ctpc_tech_cc)
...
prenatal_goi_list <- 
  ...
  fit <- lm(x ~ no2_preg_entire + sex + gestational_days +
                    ...
                    cbmc.PC4 + cbmc.PC5 + cbmc.PC6 + 
                    genotyping.PC1 + genotyping.PC2 + 
                    genotyping.PC3 + run + row + sv1 + sv2 + sv3 + sv4 + sv5 + sv6, data=combined)
...
pval <- pval2(covariates_geno_ctpc_tech_cc %>% 
                select(-no_vaccines_6mos, 
                       -clinician_wheeze_dx_y3, 
                       -reported_asthma_dx_y1, 
                       -move_6m,
...
numsv2 <- function (dat, mod, method = c("be", "leek"), vfilter = NULL, 
    B = 20, seed = NULL) 
{
    #Be
        warn <- NULL
...
sva2 <- function (dat, mod, mod0 = NULL, n.sv = NULL, controls = NULL, 
    ...
    B = 5, numSVmethod = "be") 
{
    ...
    if (n.sv > 0) {
        ...
        if (method == "irw") {
            return(irwsva.build(dat = dat, mod = mod, mod0 = mod0, 
                n.sv = n.sv, B = B))
        }
 ...
 irwsva.build2 <- function (dat, mod, mod0 = NULL, n.sv, B = 5) 
{
    n <- ncol(dat)
    m <- nrow(dat)
    if (is.null(mod0)) {
