...
```{r model_matrix}
# create model matrix
mod <- model.matrix(~ no2_preg_entire + sex + gestational_days +
...
colnames(cb_pvals_tech) <- c(paste("pval", 
                                   colnames(cb_pvals_tech)[1:22], 
                                   sep = "_"), 
                             paste("coeff", 
                                   colnames(cb_pvals_tech)[23:44], 
                                   sep = "_"),
                             colnames(cb_pvals_tech)[45:77])
...
#linear regression
cb_cg05732646_lm <- lm(cb_cg05732646 ~ no2_preg_entire + 
...
#get partial residuals for no2 and dnam 
cb_cg05732646_res <- partialResid(cb_cg05732646 ~ no2_preg_entire, cb_cg05732646_lm, 
              data=covariates_geno_ctpc_tech_cc)
...
```{r model_matrix}
# create model matrix
y1_mod <- model.matrix(~ no2_preg_entire + sex + 
...
colnames(y1_pvals_tech) <- c(paste("pval", 
                                   colnames(y1_pvals_tech)[1:29], 
                                   sep = "_"), 
                             paste("coeff", 
                                   colnames(y1_pvals_tech)[30:58], 
                                   sep = "_"),
                             colnames(y1_pvals_tech)[59:91])
...
y1_cg15325658_lm <- lm(y1_cg15325658 ~ no2_preg_entire + 
                         ...
                         y1sv6 + y1sv7 + y1sv9 + y1sv9, 
                       data=covariates_geno_ctpc_tech_cc_y1_sub)


#get partial residuals for no2 and dnam 
y1_cg15325658_res <- partialResid(y1_cg15325658 ~ no2_preg_entire, 
                                  y1_cg15325658_lm, 
                                  data=covariates_geno_ctpc_tech_cc_y1_sub)
...
covariates_geno_ctpc_tech_cc$cg15325658 <- 
  cb_betas_geno_cc_clean[rownames(cb_betas_geno_cc_clean)=="cg15325658", ] 
...
cg15325658_fit_bm <- lm(cg15325658 ~ no2_preg_entire +
                          ...
                          genotyping.PC1 + genotyping.PC2 + 
                          genotyping.PC3 + sv1 + sv2 + sv3 + sv4 + sv5 + sv6 +
                          birth_month + no2_preg_entire*birth_month, data=covariates_geno_ctpc_tech_cc)
