...
colnames(cb_pdata_ctpc)[8] <- "chip"

#rename pdata sentrix Id column to chip
colnames(cb_pdata_ctpc)[11] <- "run"

#add technical columns of pdata to covariates data
covariates_sub_ctpc_tech <- cbind(covariates_sub_ctpc, cb_pdata_ctpc[,c("run", "row", "chip")])
...
```{r sub_on_geno}
#subset covariates
covariates_ctpcs_sub <- 
  covariates_ctpcs[covariates_ctpcs$sampleid %in% rownames(genosub_order), ]
...
ggplot(cb_pvals_tech, aes(x=as.numeric(db)*100 , y=-log10(as.numeric(pval_no2_preg_entire)))) +
  ...
  ggsave( filename="2021-01-21_CHILD_cb_volcanoplot.tiff",
          device = "tiff",
          path =  here("figures", "linear_regression"),
          width = 3,
          height = 2,
...
#add dnam to covariates for linear regression
covariates_ctpcs_geno_tech_cc$cb_cg16404170<- 
...
ggplot(cb_cg16404170_res, aes(x=xresid, y=yresid)) + 
  ...
  labs(x=bquote("Prenatal" ~NO[2]~ "(ppb) partial residual"), 
       y="Methylation (%) partial residual") +
  annotate("text",x=8,y=0.06, label="slope = -0.2138", size=2.25) +
  ...
  ggsave( filename="2021-01-21_CHILD_cb_cg16404170_FAM53B.tiff",
          device = "tiff",
          path =  here("figures", "linear_regression"),
          width = 2.5,
          height = 2,
...
cb_betas_geno_cc_t <- t(cb_betas_geno_cc_sub)
