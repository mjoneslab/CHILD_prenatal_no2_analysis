...
load(here("output_data", "deconvolution", "2021-01-28_CBMC_PCs_N101.RData"))
...
Subset betas, pdata, covariates, and genotyping for cell Pcs
```{r subset}
#betas
cb_betas_ctpc <- cb_betas[, rownames(cb_pcs)]
dim(cb_betas_ctpc) # 424644    101

#pdata
cb_pdata_ctpc <- cb_pdata[cb_pdata$Sample_Label %in% rownames(cb_pcs), ]
dim(cb_pdata_ctpc) # 101 17

#covariates
covariates_sub_ctpc <- covariates_sub[covariates_sub$sampleid %in% rownames(cb_pcs), ]
dim(covariates_sub_ctpc) # 101 114

#genotyping
genosub_order_ctpc <- genosub_order[rownames(cb_pcs), ]
dim(genosub_order_ctpc) # 101 10
```
...
#pdata and pcs
identical(as.character(cb_pdata_ctpc$Sample_Label),rownames(cb_pcs)) #true

#betas and pcs
identical(colnames(cb_betas_ctpc), rownames(cb_pcs))

#covariates and pcs
identical(as.character(covariates_sub_ctpc$sampleid), rownames(cb_pcs)) #true

#geno and pcs
identical(rownames(genosub_order_ctpc), rownames(cb_pcs)) #true
```
...
cb_pdata_ctpc$row <- as.numeric(as.factor(substr(cb_pdata_ctpc$Sentrix_Position, 1, 3)))
#examine
head(cb_pdata_ctpc$row)

#rename pdata sentrix Id column to chip
colnames(cb_pdata_ctpc)[11] <- "run"

#add technical columns of pdata to covariates data
covariates_sub_ctpc_tech <- cbind(covariates_sub_ctpc, 
                                  cb_pcs,
                                  genosub_order_ctpc,
                                  cb_pdata_ctpc[,c("run", "row")])

#reclass run 
covariates_sub_ctpc_tech$run <- factor(covariates_sub_ctpc_tech$run, ordered=F)
```
...
sum(complete.cases(covariates_sub_ctpc_tech %>% 
...
cb_design <- 
  model.matrix(~no2_preg_entire + sex + gestational_days +
               ...
               data=covariates_sub_ctpc_tech)

# check size
dim(cb_design) # 101 18

# subset beta values for complete cases
cb_betas_ctpc_cc <- cb_betas_ctpc[,colnames(cb_betas_ctpc) %in% 
                                    covariates_sub_ctpc_tech$sampleid]

# check size of complete case betas
dim(cb_betas_ctpc_cc) # 424644 101

# remove probes from beta that were used to predict cell type
cb_betas_ctpc_cc_sub <- cb_betas_ctpc_cc[!rownames(cb_betas_ctpc_cc) %in% 
                                           rownames(cb_deconvolutionprobes$coefEsts), ]
# check size
dim(cb_betas_ctpc_cc_sub) # 424079    101
# 565 probes removed
...
cb_fit <-  lmFit(cb_betas_ctpc_cc_sub, cb_design)
...
ggplot(pval_qq, aes(x=e, y=o)) + 
  ...
  theme(panel.grid.major = element_blank(), 
    ...
    ggsave( filename="2021-02-02_CHILD_cb_qqplot.tiff",
          device = "tiff",
...
Delta beta
```{r db}
range = max(covariates_sub_ctpc_tech$no2_preg_entire) -
    min(covariates_sub_ctpc_tech$no2_preg_entire)
...
ggplot(cb_pvals_tech, aes(x=as.numeric(db)*100 ,
                          y=-log10(as.numeric(pval_no2_preg_entire)))) +
  ...
  ggrepel::geom_text_repel(data=subset(cb_pvals_tech, 
                                       (pval_no2_preg_entire < 0.00001 & db > 0.025) | 
                                         (pval_no2_preg_entire < 0.00001 & db < -0.025)),
                           mapping=aes(x=as.numeric(db)*100 ,
                                       y=-log10(as.numeric(pval_no2_preg_entire)),
                                       label=cpgid), size=3) + 

  scale_x_continuous(breaks=c(-25, -10, -5, -2.5, 0, 2.5, 5, 10, 25)) +
  ...
  ggsave( filename="2021-02-02_CHILD_cb_volcanoplot.tiff",
          device = "tiff",
          path =  here("figures", "linear_regression"),
          width = 3,
          height = 2,
...
covariates_ctpcs_geno_tech_cc$cb_cg15325658<- 
...
ggplot(cb_cg15325658_res, aes(x=xresid, y=yresid)) + 
  ...
  annotate("text",x=8,y=0.06, label="slope = -0.09817", size=2.25) +
  ...
  ggsave( filename="2021-01-21_CHILD_cb_cg15325658_ROBO1.tiff",
          device = "tiff",
          path =  here("figures", "linear_regression"),
          width = 2.5,
          height = 2,
...
#add dnam to covariates for linear regression
covariates_ctpcs_geno_tech_cc$cb_cg20430496<- 
...
ggplot(cb_cg20430496_res, aes(x=xresid, y=yresid)) +
  ...
  labs(x=bquote("Prenatal" ~NO[2]~ "(ppb) partial residual"), 
       y="Methylation (%) partial residual") +
  annotate("text",x=8,y=0.06, label="slope = -0.1856", size=2.25) +
  ggtitle("cg20430496") +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        axis.text = element_text(size=6),
  ...
  ggsave( filename="2021-01-21_CHILD_cb_cg20430496.tiff",
          device = "tiff",
          path =  here("figures", "linear_regression"),
          width = 2.5,
          height = 2,
...
df <- data.frame(no2 = covariates_ctpcs_geno_tech_cc$no2_preg_entire,
                 dnam = covariates_ctpcs_geno_tech_cc$cb_cg20430496,
...
covariates_ctpcs_geno_tech_cc <- cbind(covariates_ctpcs_geno_tech_cc, 
                                       y1_pdata[y1_pdata$Sample_Label %in% covariates_ctpcs_geno_tech_cc$sampleid, c("Run_y1", "Row_y1")])
...
identical(colnames(y1_betas_cc_sub), 
          as.character(covariates_ctpcs_geno_tech_cc_y1$sampleid)) # TRUE
...
#add dnam to covariates for linear regression
covariates_ctpcs_geno_tech_cc_y1$y1_cg15325658<- 
...
ggplot(y1_cg15325658_res, aes(x=xresid, y=yresid)) + 
  ...
  labs(x=bquote("Prenatal" ~NO[2]~ "(ppb) partial residual"), 
       y="Methylation (%) partial residual") +
  annotate("text",x=8,y=0.06, label="slope = -0.03209", size=2.25) +
  ggtitle("ROBO1 cg15325658") +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        axis.text = element_text(size=6),
  ...
  ggsave( filename="2021-01-21_CHILD_y1_cg15325658_ROBO1.tiff",
          device = "tiff",
          path =  here("figures", "linear_regression"),
          width = 2.5,
          height = 2,
...
#add dnam to covariates for linear regression
covariates_ctpcs_geno_tech_cc_y1$y1_cg16404170<- 
...
ggplot(y1_cg16404170_res, aes(x=xresid, y=yresid)) + 
  ...
  labs(x=bquote("Prenatal" ~NO[2]~ "(ppb) partial residual"), 
       y="Methylation (%) partial residual") +
  annotate("text",x=8,y=0.06, label="slope = -0.01538", size=2.25) +
  ...
  ggsave( filename="2021-01-21_CHILD_y1_cg16404170_FAM35B.tiff",
          device = "tiff",
          path =  here("figures", "linear_regression"),
          width = 2.5,
          height = 2,
...
#add dnam to covariates for linear regression
covariates_ctpcs_geno_tech_cc_y1$y1_cg20430496<- 
...
ggplot(y1_cg20430496_res, aes(x=xresid, y=yresid)) + 
  ...
  labs(x=bquote("Prenatal" ~NO[2]~ "(ppb) partial residual"), 
       y="Methylation (%) partial residual") +
  annotate("text",x=8,y=0.06, label="slope = -0.02204", size=2.25) +
  ggtitle("cg20430496") +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        axis.text = element_text(size=6),
  ...
  ggsave( filename="2021-01-21_CHILD_y1_cg20430496.tiff",
          device = "tiff",
          path =  here("figures", "linear_regression"),
          width = 2.5,
          height = 2,
...
```{r mean_meth}
#convert betas to df
cb_betas_geno_cc_sub_df <- as.data.frame(cb_betas_geno_cc_sub)

methmeans <- colMeans(cb_betas_geno_cc_sub)
...
cb_betas_geno_cc_lines <- 
  cb_betas_geno_cc_sub[rownames(cb_betas_geno_cc_sub) %in%
                     annotationlines$X, ]

dim(cb_betas_geno_cc_lines) # 182299 101
...
cb_betas_ctpc_cc_sub_t <- t(cb_betas_ctpc_cc_sub)


#apply function for calculating significance of associations using mice and imputed covariate data
prenatal_goi_list <- 
  pbapply(cb_betas_ctpc_cc_sub_t[,colnames(cb_betas_ctpc_cc_sub_t ) %in% rownames(goi)], 2, function(x) {
  
  combined <- cbind(covariates_sub_ctpc_tech, x)
