...
Load betas
```{r load_betas}
load(file=here("output_data", "celltype_corrected_betas", "2021-01-08_CHILD_cb_betas_5celltypePC_corrected.Rdata"))

#check ranges to confirm DNAm data is in beta value format
dim(adj_cb_betas) #424644    144
max(adj_cb_betas) #0.9999998
min(adj_cb_betas) #1.134957e-06
```
...
identical(colnames(adj_cb_betas), rownames(cb_pdata))#true
...
Examining methylation at two nominally significant CpGs in MRPS30 and STK38.
```{r nom_sig_DNAm}
...
covariates_sub_geno_tech_cc$cg24153171 <- 
  adj_cb_betas_geno_cc[rownames(adj_cb_betas_geno_cc)=="cg24153171", ] 

cg24153171_fit <- lm(cg24153171 ~ 1 + no2_preg_entire + 
                          sex + 
                          gestational_days +
                          prenatal_maternal_smoking + 
                          pss_18wk + 
...
cg24153171_res <- partialResid(cg24153171 ~ no2_preg_entire, cg24153171_fit, 
              data=covariates_sub_geno_tech_cc)

#fit again to get r2
cg24153171_res_fit <- lm(cg24153171_res$yresid ~ cg24153171_res$xresid)

summary(cg24153171_res_fit)

ggplot(cg24153171_res, aes(x=xresid, y=yresid)) + 
  geom_point(alpha=0.8, col = "#75A2D9", size=2) + 
  stat_smooth(method="lm", se=F, col="black") +
  theme_bw() +
  labs(x="Prenatal NO2 (ppb)", 
       y=str_wrap("MRPS30 cg24153171 DNA methyaltion (%)",width=40)) +
...
covariates_sub_geno_tech_cc$cg09817248 <- 
  adj_cb_betas_geno_cc[rownames(adj_cb_betas_geno_cc)=="cg09817248", ] 

cg09817248_fit <- lm(cg09817248 ~ -1 + no2_preg_entire + 
                          sex + 
                          gestational_days +
                          prenatal_maternal_smoking + 
                          pss_18wk + 
...
#get partial residuals for no2 and dnam 
cg09817248_res <- partialResid(cg09817248 ~ no2_preg_entire, cg09817248_fit, 
              data=covariates_sub_geno_tech_cc)

#fit again to get r2
cg09817248_res_fit <- lm(cg09817248_res$yresid ~ cg09817248_res$xresid)

summary(cg09817248_res_fit)

ggplot(cg09817248_res, aes(x=xresid, y=yresid)) + 
  geom_point(alpha=0.8, col = "#75A2D9", size=2) + 
  stat_smooth(method="lm", se=F, col="black") +
  theme_bw() +
  labs(x="Prenatal NO2 (ppb)", 
       y=str_wrap("STK38 cg09817248 DNA methyaltion (%)",width=47)) +
  ggtitle("Partial residual plot") +
  theme(panel.border=element_blank(),
        axis.text = element_text(size=12),
        axis.title = element_text(size=14),
