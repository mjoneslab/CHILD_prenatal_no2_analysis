...
cb_betas_lm <- pbapply(cb_betas, 1, function(x) 
  {
  cordblood <- cb_pcs[colnames(cb_betas),]
  lm(x ~ PC1 + PC2 + PC3 + PC4 + PC5 +PC6, data=cordblood, x=TRUE, na.action = NULL)
  })
...
save(adj_cb_6pcs_betas, pdata_nodups, annotation,
     file = here("output_data", "celltype_corrected_betas", "2020-12-21_CHILD_birth_betas_celltype_6PCs_corrected.Rdata"))
```
