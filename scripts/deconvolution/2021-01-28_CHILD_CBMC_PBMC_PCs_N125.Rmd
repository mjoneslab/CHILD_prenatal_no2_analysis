...
Subset cell counts for pdata and genosub
```{r cellcount_sub}
#subset cbmcs
child_fscbc_ecc2_cb_df_nodups_geno <- 
  child_fscbc_ecc2_cb_df_nodups[rownames(genosub_order), ]

#check size
dim(child_fscbc_ecc2_cb_df_nodups_geno) #131

#subset pbmcs
child_fscbc_ecc2_y1_df_sub_geno <- 
  child_fscbc_ecc2_y1_df_sub[rownames(genosub_order), ]

#check size
dim(child_fscbc_ecc2_y1_df_sub_geno) #131
```

Subset covariates for genosub data. Then get complete cases
```{r sub_covariates}
#subset on geno
...
#check size
dim(covariates_nameans_geno)

#complete cases
covariates_nameans_geno_cc <- 
  covariates_nameans_geno[complete.cases(covariates_nameans_geno[,c("maternal_education_length",
                                                                    "maternal_smoking_y1",
                                                                    "sex",
                                                                    "pss_6mos",
                                                                    "csed_6mos")]), ]

dim(covariates_nameans_geno_cc) #125
```


Subset cell types for complete cases
```{r celltype_cc}
#cord blood
child_fscbc_ecc2_cb_df_nodups_geno_cc <- 
  child_fscbc_ecc2_cb_df_nodups_geno[rownames(child_fscbc_ecc2_cb_df_nodups_geno) %in%
                                       covariates_nameans_geno_cc$sampleid, ]

dim(child_fscbc_ecc2_cb_df_nodups_geno_cc) #125 6

#age one
child_fscbc_ecc2_y1_df_sub_geno_cc <- 
  child_fscbc_ecc2_y1_df_sub_geno[rownames(child_fscbc_ecc2_y1_df_sub_geno) %in%
                                       covariates_nameans_geno_cc$sampleid, ]

dim(child_fscbc_ecc2_y1_df_sub_geno_cc) #125 6
```
...
identical(rownames(child_fscbc_ecc2_cb_df_nodups_geno_cc), 
          rownames(child_fscbc_ecc2_y1_df_sub_geno_cc)) #true

#check how many negatives in cord blood
sum(child_fscbc_ecc2_cb_df_nodups_geno_cc <= 0) #0

#clr
cb_celltypes_clr <- clr(as.matrix(child_fscbc_ecc2_cb_df_nodups_geno_cc))
...
sum(child_fscbc_ecc2_y1_df_sub_geno_cc <= 0) #0

#clr
y1_celltypes_clr <- clr(as.matrix(child_fscbc_ecc2_y1_df_sub_geno_cc))
...
save(ccc_pcs,
     ...
     file = here("output_data", "deconvolution",
                 "2021-01-28_CHILD_CBMC_PBMC_PCs_N125.Rdata"))
```
