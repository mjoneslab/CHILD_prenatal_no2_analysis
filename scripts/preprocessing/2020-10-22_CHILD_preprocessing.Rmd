...
#noob followed by bmiq
bmiq <- BMIQ(noob) 
#this is slow
#BMIQ outputs a normalized beta matrix
...
#do this for both noob normalized data and BMIQ normalized data
bmiq_goodp <- bmiq[ngoodDetP,] 
...
Remove SNP probes, XY probes, and cross-reactive probes from noob. Then subset quant rows for those in noob. 
The minfi functiosn requires a genomic methyl set - cant use with BMIQ beta matrix.
...
Subset probes in BMIQ normalized data based on cleaned data.
```{r bmiq_subset}
bmiq_clean <- bmiq_goodp[rownames(bmiq_goodp) %in% rownames(cleaned_data), ]


#check probe numbers - should be the same
dim(cleaned_data) #427705    298
dim(bmiq_clean) #427705    298

```


Get pdata, fdata, annotation, and sample names from preprocessed noob data
```{r get_methinfo}
#from noob
pdata <- pData(cleaned_data)
sample_names <- sampleNames(cleaned_data)
fdata <- featureNames(cleaned_data)
annotated <- getAnnotation(cleaned_data)
...
save(mval_batchcorr, pdata_nodups, annotated,
     file=here("output_data", "preprocessed_data", "2020-10_26_CHILD_preprocessed_betas_pdata_annotated.Rdata"))
