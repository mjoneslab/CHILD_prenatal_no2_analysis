...
### Load libraries and DNA methylation data
...
By examining probe intensities.
```{r bs_conversion_qc}
# type I probes
...
# filter out probes with a detectionP value >0.01; requires RGSet 
...
save(rgset_nobadsamp, 
     file=here("output_data", "preprocessed_data", "2021-10-07_child_rgset_nobadsamp.Rdata"))
```
...
cr_probes <- read.csv(here("input_data", "CHILD_raw_dnam_data", "chen_2013_cross_reactive_probes.csv"), header=TRUE)
...
save(betas, pdata_nodups, annotation,
     file=here("output_data", "preprocessed_data", "2021-10_07_CHILD_preprocessed_betas_pdata_annotation.Rdata"))

#write out session info
writeLines(capture.output(sessionInfo()), here("output_data", "preprocessed_data", "2021-10-07_CHILD_preprocessing_ sessionInfo.txt"))
```
...
combat_chip <- ComBat(dat=mvals, batch=pdata_nodups$Sentrix_ID, mod = mod) 
...
after_chip <- heat_scree_plot(Loadings = Loadings, Num = Num, Importance = Importance,
                          Order = Order, Categorical = meta_categorical, Continuous = NULL)
after_chip
```
...
#can't have NAs for this function -> already removed them
combat_row <- ComBat(dat=combat_chip, batch=pdata_nodups$row, mod = mod) 
...
after_chip_row <- heat_scree_plot(Loadings = Loadings, Num = Num, Importance = Importance,
                          Order = Order, Categorical = meta_categorical, Continuous = NULL)
after_chip_row
...
#save mvalues, pdata, and annotation
save(betas, mvals, pdata_nodups, annotation,
     file=here("output_data", "preprocessed_data", "2021-10_07_CHILD_preprocessed_betas_pdata_annotated_COMBAT.Rdata"))
...
#can't have NAs for this function -> already removed them
combat_run <- ComBat(dat=combat_row, batch=pdata_nodups$Run, mod = mod) 
...
after_chip_row_run <- heat_scree_plot(Loadings = Loadings, Num = Num, Importance = Importance,
                          Order = Order, Categorical = meta_categorical, Continuous = NULL)
after_chip_row_run
