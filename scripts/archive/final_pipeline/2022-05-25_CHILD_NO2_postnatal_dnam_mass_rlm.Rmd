...
MASS package  for robust linear regression 
```{r mass_rlm_no2_ewas}
# create function to run robust lm over all betas
set.seed(1234)
rlm_pn_no2 <- apply(diff_betas_geno_cc_cpr, 1, function(x){
  
  covariates_geno_ctpcs_tech_cc_not0$dnam <- as.numeric(x)
  
  # robust rlm function 
  model <- rlm(formula = dnam ~ no2_y1_entire + sex + pss_6mos +
  ...
  sig_no2 <- wald.test(Sigma = vcov(model), b = coef(model), Terms = 2)
  
  out <- c(model$coefficients,
           "converged" = model$converged,
           "sig_no2" = sig_no2$result$chi2)
  
  out
}) 

rlm_pn_no2_df <- rlm_pn_no2 %>% 
  t() %>%
  as.data.frame()

saveRDS(rlm_pn_no2_df, 
        file = here("output_data",
                    "linear_regression",
                    "2022-05-31_postnatal_rlm_pvalues.rds"))
```
...
ggsave(plot = before_qqplot, 
       filename= here("figures", 
                      "linear_regression", 
                      "postnatal", 
                      "2022-05-31_pn_uncorrected_qqplot_mass_comparison.png"),
...
ggsave(plot = after_qqplot, 
       filename= here("figures", 
                      "linear_regression", 
                      "postnatal", 
                      "2022-05-31_pn_bacon_corrected_qqplot_mass_comparison.png"),
       width = 6, height = 5, units = "cm")
...
#calculate lambda
chisq <- qchisq(1-pvector2,1)
lambda <-  median(chisq)/qchisq(0.5,1)


# qqplot of bacon corrected pvalues for main paper figure
rlm_qqplot <-  ggplot(pval_qq, aes(x=e, y=o)) + 
  geom_point(col="#254A90", size=1) + 
  geom_abline(slope = 1, intercept = 0, 
              col = "red", size=0.25, linetype="dashed") +
  labs(x = "Expected -log10(p-value)",
       y = "Observed -log10(p-value)") +
  annotate("text", label = paste("λ=", signif(lambda,3), sep=""), 
           x = 5, y = 2, size = 4, colour = "black") + 
  theme_bw() +
  theme(panel.grid = element_blank(),
        axis.text.x = element_text(size=9),
        axis.text.y = element_text(size=9),
...
ggsave(plot = rlm_qqplot, 
       filename= here("figures", 
                      "linear_regression", 
                      "postnatal", 
                      "2022-05-31_mass_rpm_pvalues_mass_comparison.png"),
       width = 6, height = 5, units = "cm")
```


Direct comparison between limma pvalues, bacon corrected pvalues, and mass rlm pvalues
```{r pvalue_comparison}

limma_mass <- ggplot(data = NULL,  
                     aes(x=diff_pvals_tech %>%
                           subset(rownames(.) %in% rownames(rlm_pn_no2_df %>%
                                                              subset(converged == 1))) %>%
                           pull(pval_no2_y1_entire), 
                         y = rlm_pn_no2_df %>%
                           subset(converged == 1) %>%
                           pull(sig_no2.P))) +
  ...
  theme_bw() +
  labs(x = "Uncorrected limma p-values", y = "Mass RLM p-values")


limma_mass_sig <- ggplot(data = NULL,  
                     aes(x=diff_pvals_tech %>%
                           subset(rownames(.) %in% rownames(rlm_pn_no2_df %>%
                                                              subset(converged == 1))) %>%
                           pull(pval_no2_y1_entire), 
...
bacon_mass <- ggplot(data = NULL,  
                     aes(x=diff_pvals_tech %>%
                           subset(rownames(.) %in% rownames(rlm_pn_no2_df %>%
                                                              subset(converged == 1))) %>%
                           pull(pval_no2_y1_entire_bacon), 
                         y = rlm_pn_no2_df %>%
                           subset(converged == 1) %>%
                           pull(sig_no2.P))) +
  geom_point(alpha = 0.5) +
  geom_abline(intercept = 0, slope = 1, colour = "red", linetype = "dashed") +
  theme_bw() +
  labs(x = "Bacon-corrected limma p-values", y="Mass RLM p-values")

bacon_mass_sig <- ggplot(data = NULL,  
                     aes(x=diff_pvals_tech %>%
                           subset(rownames(.) %in% rownames(rlm_pn_no2_df %>%
                                                              subset(converged == 1))) %>%
                           pull(pval_no2_y1_entire_bacon), 
...
Coeffecient direct comparison
```{r compare_coefficients}
limma_mass_coef <- ggplot(data = NULL,  
                          aes(x=diff_pvals_tech %>%
                                subset(rownames(.) %in% rownames(rlm_pn_no2_df %>%
                                                                   subset(converged == 1))) %>%
                                pull(coeff_no2_y1_entire), 
                              y = rlm_pn_no2_df %>%
                                subset(converged == 1) %>%
                                pull(no2_y1_entire))) +
  geom_point(alpha = 0.5) +
  geom_abline(intercept = 0, slope = 1, colour = "red", linetype = "dashed") +
  theme_bw() +
  labs(x = "Limma coefficients", y = "Mass coefficients")
limma_mass_coef
```


Combine rlm pvalues with limma
```{r combine_rlm_limma_pvals}
diff_pvals_tech_rlm <- cbind(diff_pvals_tech, 
                           rlm_pn_no2_df$no2_y1_entire,
                           rlm_pn_no2_df$sig_no2.P,
                           rlm_pn_no2_df$converged)
```
...
diff_pvals_tech_rlm$limma_db <- diff_pvals_tech_rlm$coeff_no2_y1_entire * range

diff_pvals_tech_rlm$rlm_db <- diff_pvals_tech_rlm$`rlm_pn_no2_df$no2_y1_entire` * range
```
...
volcano_plot <- ggplot(diff_pvals_tech, 
  ...
  geom_point(data=subset(diff_pvals_tech, 
                         (pval_no2_y1_entire_bacon_adj < 0.25 & limma_db > 0.025) | 
                           (pval_no2_y1_entire_bacon_adj < 0.25 & limma_db < -0.025)), 
             alpha=0.4, col = "red", size=1) + 
  scale_x_continuous(limits = c(-0.50, 0.50), 
                     breaks = seq(-0.50, 0.50, by = 0.10)) +
...
#volcano plot
volcano_plot_rlm <- ggplot(diff_pvals_tech_rlm %>% 
                             subset(`rlm_pn_no2_df$converged` == 1), 
                       aes(x=as.numeric(rlm_db), 
                           y=-log10(as.numeric(`rlm_pn_no2_df$sig_no2.P`)))) +
  geom_point(alpha=0.4, col = "#254A90", size=1) +
  # 2.5% effect size cut off
  geom_point(data=subset(diff_pvals_tech_rlm %>%
                            subset(`rlm_pn_no2_df$converged` == 1) %>%
                           mutate(padj = p.adjust(`rlm_pn_no2_df$sig_no2.P`, method = "BH")), 
                         (padj < 0.25 & rlm_db > 0.025) | 
                           (padj < 0.25 & rlm_db < -0.025)), 
             alpha=0.4, col = "red", size=1) + 
  scale_x_continuous(limits = c(-0.50, 0.50), 
                     breaks = seq(-0.50, 0.50, by = 0.10)) +
...
```{r sig_cpgs_tables}
# top 10 bacon
bacontop <-  diff_pvals_tech_rlm %>%
    subset(limma_db > 0.025 | limma_db < -0.025) %>% 
  arrange(pval_no2_y1_entire_bacon_adj) %>%
  slice_head(n=10)

write.csv(bacontop, file= here("tables",
                               "ageone",
                               "2022-05-31_pn_bacon_top10.csv"))


# top 10 mass
rlmtop <-  diff_pvals_tech_rlm %>%
  subset(`rlm_pn_no2_df$converged` ==1) %>%
  mutate(`rlm_pn_no2_df$sig_no2.P_adj` = p.adjust(`rlm_pn_no2_df$sig_no2.P`, method = "BH")) %>%
  subset(rlm_db > 0.025 | rlm_db < -0.025) %>% 
  arrange(`rlm_pn_no2_df$sig_no2.P_adj`) %>%
  slice_head(n=10)

write.csv(rlmtop, file= here("tables",
                             "ageone",
                             "2022-05-31_pn_massrlm_top10.csv"))
...
cg07472943_wheeze <- ggplot(covariates_geno_ctpcs_tech_cc_not0 %>% 
                              dplyr::select(any_wheeze_y1, cg07472943) %>%
                              na.omit(), 
                            aes(x=as.factor(any_wheeze_y1), y=cg07472943)) +
  geom_boxplot(alpha=0.6, outlier.shape = NA) +
  geom_jitter(height=0, width=0.25, alpha=0.6, col = "#254A90", size=1) +
  theme_bw() +
  theme(panel.grid = element_blank(),
        axis.title = element_blank()) +
...
ggsave(plot = cg07472943_wheeze, 
       filename = here("figures", 
                       "linear_regression", 
                       "postnatal", 
                       "2022-05-09_cg07472943_wheeze.png"),
...
# atopy logistic model 
cg19263548_atopy_log <- glm(clinician_atopy_dx_y1 ~ cg19263548 +
                              sex + pss_6mos +
                              maternal_smoking_y1 + maternal_education_length +
                              celltype.PC1 + celltype.PC2 + celltype.PC3 + 
                              celltype.PC4 + celltype.PC5 + celltype.PC6 + 
