...
library(aod) # wald test for robust regression
```
...
Load genotyping PCs. Subset out REEGLE PCs (genotyping data contains PCs for all CHILD participants that were genotyped).
```{r load_genotyping_pcs}
...
```{r subset_beta_pdata_covariates_for_participants_w_genotyping}
# subset betas
...
```{r combine_covariates_genotype_celltypepcs_pdata}
...
Remove rows where air pollution is 0. A value of 0 is not realistics
```{r remove_zeroes_in_no2}
...
```{r subset_for_covariate_complete_cases}
...
Surrogate variable analysis using the SVA package. This will allow us to capture variance due to batch effects, as well as variance due to unknown biological variables. 
```{r sva}
...
set.seed(1234)
rlm_cb_no2 <- apply(cb_betas_geno_cc_clean, 1, function(x){
  
  covariates_geno_ctpc_tech_cc$dnam <- as.numeric(x)
  
  # robust rlm function 
  model <- rlm(formula = dnam ~ no2_preg_entire + sex + gestational_days + pss_18wk +
                 prenatal_maternal_smoking + maternal_education_length + 
                 cbmc.PC1 + cbmc.PC2 + cbmc.PC3 + cbmc.PC4  + 
                 genotyping.PC1 + genotyping.PC2 + genotyping.PC3 + 
...
rlm_cb_no2_df <- rlm_cb_no2 %>% 
  t() %>%
  as.data.frame()

saveRDS(rlm_cb_no2_df, 
        file = here("output_data",
                    "linear_regression",
                    "2022-05-25_prenatal_rlm_pvalues.rds"))
```
...
ggsave(plot = before_qqplot, 
       filename= here("figures", 
                      "linear_regression", 
                      "prenatal", 
                      "2022-05-25_uncorrected_qqplot_rlm_comparison.png"),
...
ggsave(plot = after_qqplot, 
       filename= here("figures", 
                      "linear_regression", 
                      "prenatal", 
                      "2022-05-25_bacon_corrected_qqplot_rlm_comparison.png"),
       width = 6, height = 5, units = "cm")
...
ggsave(plot = rlm_qqplot, 
       filename= here("figures", 
                      "linear_regression", 
                      "prenatal", 
                      "2022-05-25_mass_rpm_pvalues_rlm_comparison.png"),
       width = 6, height = 5, units = "cm")
```
...
limma_mass <- ggplot(data = NULL,  
                     aes(x=cb_pvals_tech %>%
                           subset(rownames(.) %in% rownames(rlm_cb_no2_df %>%
                                                              subset(converged == 1))) %>%
                           pull(pval_no2_preg_entire), 
                         y = rlm_cb_no2_df %>%
                           subset(converged == 1) %>%
                           pull(sig_no2.P))) +
...
bacon_mass <- ggplot(data = NULL,  
                     aes(x=cb_pvals_tech %>%
                           subset(rownames(.) %in% rownames(rlm_cb_no2_df %>%
                                                              subset(converged == 1))) %>%
                           pull(pval_no2_preg_entire_bacon), 
                         y = rlm_cb_no2_df %>%
                           subset(converged == 1) %>%
                           pull(sig_no2.P))) +
  geom_point(alpha = 0.5) +
...
limma_mass_coef <- ggplot(data = NULL,  
                          aes(x=cb_pvals_tech %>%
                                subset(rownames(.) %in% rownames(rlm_cb_no2_df %>%
                                                                   subset(converged == 1))) %>%
                                pull(coeff_no2_preg_entire), 
                              y = rlm_cb_no2_df %>%
                                subset(converged == 1) %>%
                                pull(no2_preg_entire))) +
  geom_point(alpha = 0.5) +
  geom_abline(intercept = 0, slope = 1, colour = "red", linetype = "dashed") +
  theme_bw() +
...
cb_pvals_tech_rlm <- cbind(cb_pvals_tech, 
                           rlm_cb_no2_df$no2_preg_entire,
                           rlm_cb_no2_df$sig_no2.P,
                           rlm_cb_no2_df$converged)
```
...
cb_pvals_tech_rlm$limma_db <- cb_pvals_tech_rlm$coeff_no2_preg_entire * range
cb_pvals_tech_rlm$mass_db <- cb_pvals_tech_rlm$`rlm_cb_no2_df$no2_preg_entire` * range
```
...
# plot bacon-corrected pvalue volcano
cb_volcano <- ggplot(cb_pvals_tech, 
  ...
  geom_point(data=subset(cb_pvals_tech, 
                         (pval_no2_preg_entire_bacon_adj < 0.25 & limma_db > 0.025) | 
                           (pval_no2_preg_entire_bacon_adj < 0.25 & limma_db < -0.025)), 
...
# plot MASS RLM volcano
cb_volcano_rlm <- ggplot(cb_pvals_tech_rlm %>%
                           subset(`rlm_cb_no2_df$converged`==1), 
                         aes(x=as.numeric(mass_db) , 
                             y=-log10(as.numeric(`rlm_cb_no2_df$sig_no2.P`)))) +
  geom_point(alpha=0.4, col = "#254A90", size=1) +
  # colour significant cpgs; 2.5% effect size cut off and FDR < 0.25
  geom_point(data=subset(cb_pvals_tech_rlm %>% 
                           subset(`rlm_cb_no2_df$converged`==1) %>%
                           mutate(adjp  = p.adjust(`rlm_cb_no2_df$sig_no2.P`, method = "BH")), 
                         (adjp < 0.25 & mass_db > 0.025) | 
                           (adjp < 0.25 & mass_db < -0.025)), 
             alpha=0.4, col = "red", size=1) + 
  scale_x_continuous(breaks = seq(-0.50, 0.50, by = 0.10)) +
  labs(y="-log10(unadjusted p-value)", x = "Effect size (% DNAm)") +
...
write.csv(bacontop, file= here("tables",
                               "prenatal",
                               "2022-05-31_bacon_top10.csv"))


# top 10 mass
rlmtop <-  cb_pvals_tech_rlm %>%
  subset(`rlm_cb_no2_df$converged` ==1) %>%
  mutate(`rlm_cb_no2_df$sig_no2.P_adj` = p.adjust(`rlm_cb_no2_df$sig_no2.P`, method = "BH")) %>%
  subset(mass_db > 0.025 | mass_db < -0.025) %>% 
  arrange(`rlm_cb_no2_df$sig_no2.P_adj`) %>%
  slice_head(n=10)

write.csv(rlmtop, file= here("tables",
                             "prenatal",
                             "2022-05-31_massrlm_top10.csv"))
