...
#can use limma to get residuals
#this is quicker than lm()
#limma uses lower level function lm.series, which calls lm.fit (used by lm())
cb_design <- model.matrix(~
              Row + Chip,
              data=cb_pdata)

#fit methylation to batch variables then get residuals
cb_fit <-  lmFit(cb_betas, cb_design)

#get residuals 
cb_residuals <- residuals(cb_fit, cb_betas) 
...
cb_betas_batchcorr <- cb_residuals + matrix(apply(cb_betas, 1, mean), 
...
#adjusted
print(cb_betas_batchcorr[1:5,1:5])

#check min and max of adjsuted data
min(cb_betas_batchcorr) # -0.3523001
max(cb_betas_batchcorr) # 1.286878

# cant have values > 1 or < 0
# dont want to fit PCs to this impossible data
...
cb_betas_batchcorr[cb_betas_batchcorr > 1] <- max(cb_betas_batchcorr[cb_betas_batchcorr < 1])
cb_betas_batchcorr[cb_betas_batchcorr < 0] <- min(cb_betas_batchcorr[cb_betas_batchcorr > 0])

#check that adjusted betas do not fall below 0 or above 1
max(cb_betas_batchcorr) # 0.9999976
min(cb_betas_batchcorr) # 6.357526e-06

#cb_betas_batchcorr <- pbapply(cb_betas, 1, function(x) 
#  {
#  cb_batch <- cb_pdata[colnames(cb_betas),]
#  lm(x ~ Run + Row, 
#     data=cb_batch, 
#     x=TRUE, 
#     na.action = NULL)
...
#cb_residuals <- t(pbsapply(cb_betas_batchcorr, function(x) residuals(summary(x))))



identical(test, cb_residuals)

matequal <- function(x, y) {is.matrix(x) && is.matrix(y) && dim(x) == dim(y) && all(x == y)}


max(abs(test - cb_residuals))

test_melted <- as.vector(test)

cb_residuals_melted <- as.vector(cb_residuals)

dftest <- as.data.frame(cbind(test_melted=test_melted, 
                cb_residuals_melted = cb_residuals_melted))

ggplot(dftest, aes(x=test_melted, y=cb_residuals_melted)) + 
  geom_point() +
...
#fit methylation to batch variables then get residuals
cb_pcs_fit <-  lmFit(cb_betas_batchcorr, cb_pcs_design)

#get residuals 
cb_pcs_residuals <- residuals(cb_pcs_fit, cb_betas_batchcorr) 

#check if colnames of cordblood residuals and cord blood batch corrected betas are the same
identical(colnames(cb_pcs_residuals), colnames(cb_betas_batchcorr))
...
PCs after correction for batch variables
```{r heatscree_after_batch}
#convert unadjusted betas to mvals for pc heat scree
cb_mvals_batch <- lumi::beta2m(cb_betas_batchcorr)

#heat scree
uncor.dat <- t(scale(t(cb_mvals_batch)))
PCA_full <- princomp(na.omit(uncor.dat))
Loadings <- as.data.frame(unclass(PCA_full$loadings))
vars <- PCA_full$sdev^2
Importance <- vars/sum(vars)
...
save(adj_cb_betas, pdata_nodups, annotation,
     file = here("output_data", "celltype_corrected_betas", "2021-01-08_CHILD_cb_betas_5celltypePC_chiprow_corrected.Rdata"))
```
