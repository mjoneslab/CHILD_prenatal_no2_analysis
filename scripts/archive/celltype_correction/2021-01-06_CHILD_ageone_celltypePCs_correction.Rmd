...
Correct cord blood DNAm data and year one DNAm data for batch effects (run and row) using before subtraction using meaghan's linear regression method.
```{r batch_corr}
...
cb_betas_batchcorr <- pbapply(cb_betas, 1, function(x) 
  {
  cb_batch <- cb_pdata[as.character(cb_pdata$Sample_Label) %in% colnames(cb_betas),]
  lm(x ~ Run + Row, 
     data=cb_batch, 
     x=TRUE, 
     na.action = NULL)
...
y1_betas_batchcorr <- pbapply(diff_betas, 1, function(x) 
  {
  diff_pcs <- ccc_pcs[colnames(diff_betas),]
  lm(x ~ PC1  + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9, 
     data=diff_pcs, 
     x=TRUE, 
     na.action = NULL)
  })
...
diff_betas_lm <- pbapply(diff_betas, 1, function(x) 
  {
  diff_pcs <- ccc_pcs[colnames(diff_betas),]
  lm(x ~ PC1  + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9, 
     data=diff_pcs, 
...
diff_beta_residuals <- t(pbsapply(diff_betas_lm, function(x) residuals(summary(x))))


#check if colnames of cordblood residuals and cord blood beta are the same
identical(colnames(diff_beta_residuals), colnames(diff_betas))
...
adj_diff_betas <- diff_beta_residuals + matrix(apply(diff_betas, 1, mean), 
                                     nrow=nrow(diff_beta_residuals), 
                                     ncol=ncol(diff_beta_residuals))
...
max(adj_diff_betas) #1.156157
min(adj_diff_betas) #-0.2391103
...
y1_pdata_sub$Sentrix_ID <- as.factor(y1_pdata_sub$Sentrix_ID)
#pull out row from sentrix position then set to factor
y1_pdata_sub$row <- substr(y1_pdata_sub$Sentrix_Position, 1, 3)
...
identical(rownames(y1_pdata_sub), rownames(child_fscbc_ecc2_y1_df))
#bind columns together for one large metadata frame
y1_pdata_sub <- cbind(y1_pdata_sub, child_fscbc_ecc2_y1_df)
