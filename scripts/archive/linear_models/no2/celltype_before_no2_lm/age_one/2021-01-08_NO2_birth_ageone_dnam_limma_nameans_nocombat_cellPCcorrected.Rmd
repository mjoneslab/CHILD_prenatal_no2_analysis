...
Load  betas data
```{r covariates}
load("R:/Jones/People/Samantha Lee/Projects/CHILD/CHILD_air_pollution/output_data/celltype_corrected_betas/2021-01-08_CHILD_diff_betas_celltype9PCs_corrected.Rdata")
```
...
identical(as.character(cb_pdata_geno$Sample_Label), 
          as.character(covariates_sub_geno$sampleid)) #true
...
covariates_sub_geno_tech <- cbind(covariates_sub_geno, 
...
sum(complete.cases(covariates_sub_geno_tech %>% 
...
covariates_sub_geno_tech_cc <- 
  covariates_sub_geno_tech[covariates_sub_geno_tech %>% 
                             dplyr::select(no2_y1_entire, sex,  
                                           maternal_smoking_y1,  
                                           maternal_education_length, 
                                           pss_6mos,
...
#check size
dim(covariates_sub_geno_tech_cc) # 125 140 

#subset betas
diff_betas_geno_cc <- 
  diff_betas_geno[,colnames(diff_betas_geno) %in% covariates_sub_geno_tech_cc$sampleid]
#check size
...
colnames(diff_pvals_tech) <- c(paste("pval", colnames(diff_pvals_tech)[1:17], sep = "_"), 
                         paste("coeff", colnames(diff_pvals_tech)[18:34], sep = "_"),
                         colnames(diff_pvals_tech)[35:67])
...
p <- diff_pvals_tech$pval_no2_y1_entire

hist(p, breaks=seq(0, 1, b), col="gray", border="white")

# Higher criticism:

abline(h=qbinom(0.95, length(p), b), col="red")

# Quality control (bonferonni)(:
...
#get slope
lmstats <- pbapply(adj_diff_betas_geno_cc_t, 2, function(x) {
...
#disregard cpg column of lmstats wide - it is incorrect
