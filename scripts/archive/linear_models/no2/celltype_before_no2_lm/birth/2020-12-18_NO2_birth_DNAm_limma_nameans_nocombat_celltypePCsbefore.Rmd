...
Load betas
```{r load_betas}
load(file=here("output_data", "celltype_corrected_betas", "2020-12-18_CHILD_birth_betas_celltype_PCs_corrected.Rdata"))

#check ranges to confirm DNAm data is in beta value format
dim(adj_cb_pcs_betas) #424644    144
max(adj_cb_pcs_betas) #0.9999994
min(adj_cb_pcs_betas) #8.904845e-06
```
...
identical(colnames(adj_cb_pcs_betas), cb_pdata$sentrix_name)#true
#rename colnames of beta values to sample label
colnames(adj_cb_pcs_betas) <- cb_pdata$Sample_Label
```
...
```{r sub_covariates}
covariates_sub <- covariates_nameans[as.character(covariates_nameans$sampleid) %in% colnames(adj_cb_pcs_betas), ]

#check if they are in the same order
identical(as.character(covariates_sub$sampleid), colnames(adj_cb_pcs_betas)) #true
```
...
identical(as.character(cb_pdata$Sample_Label), colnames(adj_cb_pcs_betas)) #true
...
adj_cb_pcs_betas_geno <- adj_cb_pcs_betas[,colnames(adj_cb_pcs_betas) %in% rownames(genosub_order)] 
dim(adj_cb_pcs_betas_geno) #131

#check that this is in the same order as covariates
identical(as.character(covariates_sub_geno$sampleid), colnames(adj_cb_pcs_betas_geno)) #true
...
adj_cb_mvals <- lumi::beta2m(adj_cb_pcs_betas_geno_cc)
...
#subset betas
adj_cb_pcs_betas_geno_cc <- 
  adj_cb_pcs_betas_geno[,colnames(adj_cb_pcs_betas_geno) %in% covariates_sub_geno_tech_cc$sampleid]

#check size
dim(adj_cb_pcs_betas_geno_cc) #101

#fit methylation to no2
cb_fit <-  lmFit(adj_cb_pcs_betas_geno_cc, cb_design)
...
adj_cb_pcs_betas_geno_cc_t <- t(adj_cb_pcs_betas_geno_cc)

#get slope
lmstats <- pbapply(adj_cb_pcs_betas_geno_cc_t, 2, function(x) {
...
write.csv(lmstats_wide, file=here("output_data", "linear_regression", "2020-12-18_birth_deltabeta_celltypePCsbefore.csv"))
...
compare
```{r compare}
pvals <- as.data.frame(cbind(limma = cb_pvals_tech$coeff_no2_preg_entire,
               lm = cb_pvals_tech$db/range))

ggplot(pvals, aes(x=limma, y=lm)) + 
  geom_point() +
...
covariates_sub_geno_tech_cc$cg20701183 <- 
  adj_cb_pcs_betas_geno_cc[rownames(adj_cb_pcs_betas_geno_cc)=="cg20701183", ] 
...
covariates_sub_geno_tech_cc$cg12076102 <- 
  adj_cb_pcs_betas_geno_cc[rownames(adj_cb_pcs_betas_geno_cc)=="cg12076102", ] 
...
covariates_sub_geno_tech_cc$cg16928046 <- 
  adj_cb_pcs_betas_geno_cc[rownames(adj_cb_pcs_betas_geno_cc)=="cg16928046", ] 
...
```{r mean_meth}
#convert betas to df
adj_cb_pcs_betas_geno_cc_df <- as.data.frame(adj_cb_pcs_betas_geno_cc)

methmeans <- colMeans(adj_cb_pcs_betas_geno_cc_df)
...
#subset betas
adj_cb_pcs_betas_geno_cc_lines <- 
  adj_cb_pcs_betas_geno_cc[rownames(adj_cb_pcs_betas_geno_cc) %in%
                     annotationlines$X, ]

dim(adj_cb_pcs_betas_geno_cc_lines)


#convert betas to df
adj_cb_pcs_betas_geno_cc_lines_df <- as.data.frame(adj_cb_pcs_betas_geno_cc_lines)

linemeth <- colMeans(adj_cb_pcs_betas_geno_cc_lines)
...
prenatal_goi_list <- 
  pbapply(adj_cb_pcs_betas_geno_cc_t[,colnames(adj_cb_pcs_betas_geno_cc_t) %in% rownames(goi)], 2, function(x) {
...
covariates_sub_geno_tech_cc$cg26227465_cb <- 
        adj_cb_pcs_betas_geno_cc[rownames(adj_cb_pcs_betas_geno_cc)=="cg26227465", ]
...
covariates_sub_geno_tech_cc$cg12283362 <- 
  adj_cb_pcs_betas_geno_cc[rownames(adj_cb_pcs_betas_geno_cc)=="cg12283362", ] 
...
covariates_sub_geno_tech_cc$cg08973675 <- 
  adj_cb_pcs_betas_geno_cc[rownames(adj_cb_pcs_betas_geno_cc)=="cg08973675", ] 
...
#apply function for calculating significance of associations using mice and imputed covariate data
ifnl1_list <- 
  pbapply(adj_cb_pcs_betas_geno_cc_t [,colnames(adj_cb_pcs_betas_geno_cc_t ) %in% rownames(ifnl1_cpgs)], 2, function(x) {
...
covariates_sub_geno_tech_cc$cg07424400 <- 
  adj_cb_pcs_betas_geno_cc[rownames(adj_cb_pcs_betas_geno_cc)=="cg07424400", ] 
