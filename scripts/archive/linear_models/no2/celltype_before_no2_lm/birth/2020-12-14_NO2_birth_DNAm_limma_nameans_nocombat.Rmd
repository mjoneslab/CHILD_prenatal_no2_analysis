...
Load betas
```{r load_betas}
load(file=here("output_data", "celltype_corrected_betas",
               "2020-12-14_CHILD_birth_betas_celltype_corrected.Rdata"))

#check ranges to confirm DNAm data is in beta value format
dim(adj_cb_betas) #424644    144
...
identical(colnames(adj_cb_betas), cb_pdata$sentrix_name)#true
#rename colnames of beta values to sample label
colnames(adj_cb_betas) <- cb_pdata$Sample_Label
```
...
```{r sub_covariates}
covariates_sub <- covariates_nameans[as.character(covariates_nameans$sampleid) %in% colnames(adj_cb_betas), ]

#check if they are in the same order
identical(as.character(covariates_sub$sampleid), colnames(adj_cb_betas)) #true
```
...
identical(as.character(cb_pdata$Sample_Label), colnames(adj_cb_betas)) #true
...
adj_cb_betas_geno <- adj_cb_betas[,colnames(adj_cb_betas) %in% rownames(genosub_order)] 
dim(adj_cb_betas_geno) #131

#check that this is in the same order as covariates
identical(as.character(covariates_sub_geno$sampleid), colnames(adj_cb_betas_geno)) #true
...
adj_cb_mvals <- lumi::beta2m(adj_cb_betas_geno_cc)
...
meta_categorical <- covariates_sub_geno_tech_cc[,c("sex", "run")] 
meta_continuous <- covariates_sub_geno_tech_cc[,c("no2_preg_entire", 
                                                  "gestational_days", 
    ...
    if(class(df[[pos_1]]) %in% c("factor", "character") &&
      ...
      if(is.factor(tmp2[,1]) && (nlevels(droplevels(tmp2[,1]))<2)){
        ...
        if(is.factor(tmp2[,2]) && (nlevels(droplevels(tmp2[,2]))<2)){
          ...
          if(( colnames(df)[[pos_1]] == colnames(df)[[pos_1]] )) {
            p = 1
          } else {
          #get p value
          fish_table <- table(df[[pos_1]], df[[pos_2]])
...
covariates_sub_geno_tech_cc_heat <- 
  subset(covariates_sub_geno_tech_cc, 
         select=c(no2_preg_entire, sex, gestational_days,
                  prenatal_maternal_smoking, prenatal_shs,
                  pss_18wk, csed_18wk, 
                  maternal_education_length,
                  maternal_ethnicity, 
                  ...
                  studycentre,
                  pests_18wk, pets_18wk, mould_18wk, leaks_18wk,attached_garage_18wk))



#pval calcs
covariates_pval <- pval2(covariates_sub_geno_tech_cc_heat)

#make the matrix a dataframe
covariates_pval_df <- as.data.frame(covariates_pval)

#set all nonsig values to NA
sum(covariates_pval_df==999) #0
...
lower_tri_pval <- get_lower_tri(covariates_pval_df)
...
#order levels according to order of row names of the lower triangle of the matrix
melted_pmat$covariates1 <- as.factor(melted_pmat$covariates1)
melted_pmat$covariates1 <- factor(melted_pmat$covariates1,
                                  levels=c("no2_preg_entire", "sex", "gestational_days",
                                           "prenatal_maternal_smoking", "prenatal_shs", 
                                           "pss_18wk", "csed_18wk", "maternal_education_length",
                                           "maternal_ethnicity", "maternal_asthma",
                                           "clinician_asthma_dx_5y", "genotyping.PC1",
                                           "genotyping.PC2", "genotyping.PC3", "studycentre",
                                           "pests_18wk", "pets_18wk", "mould_18wk", 
                                           "leaks_18wk", "attached_garage_18wk"))
...
#correlation calcs
covariates_corr <- cor2(covariates_sub_geno_tech_cc_heat)

#check missing
sum(is.na(covariates_corr)) #0

#make the matrix a dataframe
covariates_corr_df <- as.data.frame(covariates_corr)
...
#get the upper triangle of the covariate correlation matrix
lower_tri_corr <- get_lower_tri(covariates_corr_df)

#make a new column for covariate names - this is needed for putting data in long format
lower_tri_corr$covariates1 <- rownames(lower_tri_corr)


#melt the matrix to long format
melted_cormat <- reshape2::melt(lower_tri_corr, 
                                na.rm = TRUE,
                                id.vars="covariates1",
                                variable.name="covariates2",
...
ggplot(data = melted_cormat, aes(x = covariates1, y = covariates2, fill = correlation)) +
  ...
  scale_x_discrete(labels=c("no2_preg_entire", "sex", "gestational_days",
                                           "prenatal_maternal_smoking", "prenatal_shs", 
                                           "pss_18wk", "csed_18wk", "maternal_education_length",
                                           "maternal_ethnicity", "maternal_asthma",
                                           "clinician_asthma_dx_5y", "genotyping.PC1",
                                           ...
                                           "pests_18wk", "pets_18wk", "mould_18wk", 
                                           "leaks_18wk", "attached_garage_18wk")) +
    scale_y_discrete(labels=c("no2_preg_entire", "sex", "gestational_days",
                                           "prenatal_maternal_smoking", "prenatal_shs", 
                                           "pss_18wk", "csed_18wk", "maternal_education_length",
                                           "maternal_ethnicity", "maternal_asthma",
                                           "clinician_asthma_dx_5y", "genotyping.PC1",
...
#subset betas
adj_cb_betas_geno_cc <- 
  adj_cb_betas_geno[,colnames(adj_cb_betas_geno) %in% covariates_sub_geno_tech_cc$sampleid]

#check size
dim(adj_cb_betas_geno_cc) #101

#fit methylation to no2
cb_fit <-  lmFit(adj_cb_betas_geno_cc, cb_design)
...
adj_cb_betas_geno_cc_t <- t(adj_cb_betas_geno_cc)

#get slope
lmstats <- pbapply(adj_cb_betas_geno_cc_t, 2, function(x) {
...
write.csv(lmstats_wide, file=here("output_data", "linear_regression", "2020-12-17_birth_deltabeta_celltypebefore.csv"))

#check for same order
identical(rownames(lmstats_wide), rownames(cb_pvals_tech)) #true

#add to limma
cb_pvals_tech$db <- lmstats_wide$db

#compare slope from limma vs lm 
#cb_pvals_tech$slope = cb_pvals_tech$db/(max(covariates_sub_geno_tech_cc$no2_preg_entire) -        #                                    min(covariates_sub_geno_tech_cc$no2_preg_entire))

#ggplot(cb_pvals_tech, aes(x=coeff_no2_preg_entire, y=slope)) + geom_point()
```


compare
```{r compare}
pvals <- as.data.frame(cbind(celltypebefore = cb_pvals_celltypebefore$pval_no2_preg_entire,
               celltypein = cb_pvals_tech$pval_no2_preg_entire))

ggplot(pvals, aes(x=celltypein, y=celltypebefore)) + 
  geom_point() +
...
db <- as.data.frame(cbind(celltypebefore = cb_pvals_celltypebefore$db,
               celltypein = cb_pvals_tech$db))

ggplot(db, aes(x=celltypein, y=celltypebefore)) + 
  geom_point() +
  geom_abline(slope=1, intercept=0, linetype="dashed", col="red") +
  theme_bw() +
  theme(panel.border = element_blank()) +
...
covariates_sub_geno_tech_cc$cg20701183 <- 
  adj_cb_betas_geno_cc[rownames(adj_cb_betas_geno_cc)=="cg20701183", ] 
...
covariates_sub_geno_tech_cc$cg12076102 <- 
  adj_cb_betas_geno_cc[rownames(adj_cb_betas_geno_cc)=="cg12076102", ] 
...
covariates_sub_geno_tech_cc$cg16928046 <- 
  adj_cb_betas_geno_cc[rownames(adj_cb_betas_geno_cc)=="cg16928046", ] 
...
```{r mean_meth}
#convert betas to df
adj_cb_betas_geno_cc_df <- as.data.frame(adj_cb_betas_geno_cc)

methmeans <- colMeans(adj_cb_betas_geno_cc_df)
...
#subset betas
adj_cb_betas_geno_cc_lines <- 
  adj_cb_betas_geno_cc[rownames(adj_cb_betas_geno_cc) %in%
                     annotationlines$X, ]

dim(adj_cb_betas_geno_cc_lines)


#convert betas to df
adj_cb_betas_geno_cc_lines_df <- as.data.frame(adj_cb_betas_geno_cc_lines)

linemeth <- colMeans(adj_cb_betas_geno_cc_lines)
...
prenatal_goi_list <- 
  pbapply(adj_cb_betas_geno_cc_t[,colnames(adj_cb_betas_geno_cc_t) %in% rownames(goi)], 2, function(x) {
...
#write as .csv
write.csv(goi_df, file=here("output_data", "2020-12-17_prenatal_goi.csv"))




covariates_sub_geno_tech_cc$cg26227465_cb <- 
        adj_cb_betas_geno_cc[rownames(adj_cb_betas_geno_cc)=="cg26227465", ]
...
covariates_sub_geno_tech_cc$cg12283362 <- 
  adj_cb_betas_geno_cc[rownames(adj_cb_betas_geno_cc)=="cg12283362", ] 
...
covariates_sub_geno_tech_cc$cg08973675 <- 
  adj_cb_betas_geno_cc[rownames(adj_cb_betas_geno_cc)=="cg08973675", ] 
...
#apply function for calculating significance of associations using mice and imputed covariate data
ifnl1_list <- 
  pbapply(adj_cb_betas_geno_cc_t [,colnames(adj_cb_betas_geno_cc_t ) %in% rownames(ifnl1_cpgs)], 2, function(x) {
...
covariates_sub_geno_tech_cc$cg07424400 <- 
  adj_cb_betas_geno_cc[rownames(adj_cb_betas_geno_cc)=="cg07424400", ] 
