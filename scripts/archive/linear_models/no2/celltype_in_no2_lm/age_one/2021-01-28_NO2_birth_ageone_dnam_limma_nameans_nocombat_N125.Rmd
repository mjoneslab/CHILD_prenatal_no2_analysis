...
genosub <- genotyping[rownames(genotyping) %in% colnames(diff_betas),]
...
```{r load_celltype_PCs}
load(here("output_data", "deconvolution", "2021-01-28_CHILD_CBMC_PBMC_PCs_N125.Rdata"))
...
```{r subset}
#betas
diff_betas_ctpc <- diff_betas[, rownames(ccc_pcs)]
dim(diff_betas_ctpc) # 424644    125

#pdata
y1_pdata_ctpc <- y1_pdata[y1_pdata$Sample_Label %in% rownames(ccc_pcs), ]
dim(y1_pdata_ctpc) # 125 17

#covariates
covariates_ctpc <- covariates_nameans[covariates_nameans$sampleid %in% rownames(ccc_pcs), ]
dim(covariates_ctpc) # 125 113

#genotyping
genosub_order_ctpc <- genosub_order[rownames(ccc_pcs), ]
dim(genosub_order_ctpc) # 125 10
```
...
#### add row run and chip to y1 pdata

#create new column for technical variables in y1 pdata
y1_pdata_ctpc$y1_chip <- as.factor(y1_pdata_ctpc$Sentrix_ID)
y1_pdata_ctpc$y1_run <- as.factor(y1_pdata_ctpc$Run)
y1_pdata_ctpc$y1_row <- as.numeric(as.factor(substr(y1_pdata_ctpc$Sentrix_Position, 1, 3)))

y1_pdata_ctpc$cb_chip <- as.factor(cb_pdata[cb_pdata$Sample_Label %in%
                                              y1_pdata_ctpc$Sample_Label, "Sentrix_ID"])
y1_pdata_ctpc$cb_run <- as.factor(cb_pdata[cb_pdata$Sample_Label %in%
                                              y1_pdata_ctpc$Sample_Label, "Run"])
y1_pdata_ctpc$cb_row <- as.numeric(as.factor(substr(cb_pdata[cb_pdata$Sample_Label %in%
                                              y1_pdata_ctpc$Sample_Label, 
                                              "Sentrix_Position"], 1, 3)))

#add everything together
covariates_ctpc_geno_tech <- cbind(covariates_ctpc,
                                    genosub_order_ctpc,
                                    ccc_pcs,
                                    y1_pdata_ctpc[,c("cb_run", "cb_row", "cb_chip",
                                                     "y1_run", "y1_row", "y1_chip")])


dim(covariates_ctpc_geno_tech) #125 140
```
...
sum(complete.cases(covariates_ctpc_geno_tech %>% 
...
diff_design <- 
  model.matrix(~no2_y1_entire + sex +
                 ...
                 cb_run+ cb_row+ 
                 y1_row+ y1_run, 
               data=covariates_ctpc_geno_tech)
...
#subset betas
diff_betas_ctpc_cc <- 
  diff_betas_ctpc[,colnames(diff_betas_ctpc) %in% 
                    covariates_ctpc_geno_tech$sampleid]

#check size
dim(diff_betas_ctpc_cc) # 424644    125

#remove betas that were used for cell type prediction
diff_betas_ctpc_cc_sub <- 
  diff_betas_ctpc_cc[!rownames(diff_betas_ctpc_cc) %in% rownames(cb_deconvolutionprobes$coefEsts) & !rownames(diff_betas_ctpc_cc) %in% rownames(y1_deconvolutionprobes$coefEsts), ]

#check size
dim(diff_betas_ctpc_cc_sub) #424079    125
#565 probes removed

#check that covariates and betas in same order
identical(colnames(diff_betas_ctpc_cc_sub), 
          as.character(covariates_ctpc_geno_tech$sampleid)) # TRUE
...
diff_fit <-  lmFit(diff_betas_ctpc_cc_sub, diff_design)
...
Delta beta
```{r db}
range = max(covariates_ctpc_geno_tech$no2_y1_entire) -
    min(covariates_ctpc_geno_tech$no2_y1_entire)
...
covariates_ctpc_geno_tech$cg24535439 <- diff_betas_geno_cc[rownames(diff_betas_geno_cc)=="cg24535439", ]
...
cg24535439_lm <- lm(cg24535439  ~ no2_y1_entire + sex +
                 ...
                 celltype.PC9+
                 cb_run+ cb_row+ 
                 y1_row+ y1_run, data=covariates_ctpc_geno_tech)
...
covariates_ctpc_geno_tech$cg24727343 <- diff_betas_geno_cc[rownames(diff_betas_geno_cc)=="cg24727343", ]
...
covariates_ctpc_geno_tech$cg19867107 <- diff_betas_geno_cc[rownames(diff_betas_geno_cc)=="cg19867107", ]
...
covariates_ctpc_geno_tech$cg14916079 <- diff_betas_geno_cc[rownames(diff_betas_geno_cc)=="cg14916079", ]
...
covariates_ctpc_geno_tech$cg22365167 <- diff_betas_geno_cc[rownames(diff_betas_geno_cc)=="cg22365167", ]
...
covariates_ctpc_geno_tech$cg16955074 <- diff_betas_geno_cc[rownames(diff_betas_geno_cc)=="cg16955074", ]
...
covariates_ctpc_geno_tech$cg18668679 <- diff_betas_geno_cc[rownames(diff_betas_geno_cc)=="cg18668679", ]
...
y1_dnam <- covariates_ctpc_geno_tech[,c("cg24535439", "cg24727343", "cg19867107",
...
df <- data.frame(no2 = covariates_ctpc_geno_tech$no2_y1_entire,
                 dnam = covariates_ctpc_geno_tech$cg18668679,
...
#transpose betas
diff_betas_ctpc_cc_sub_t <- t(diff_betas_ctpc_cc_sub)

#apply function for calculating significance of associations using mice and imputed covariate data
yearone_goi_list <- 
  pbapply(diff_betas_ctpc_cc_sub_t [,colnames(diff_betas_ctpc_cc_sub_t ) %in% rownames(goi)], 2, function(x) {
  
  combined <- cbind(covariates_ctpc_geno_tech, x)
