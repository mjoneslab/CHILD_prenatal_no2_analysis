...
load(here("output_data", "deconvolution",  "2021-06-06_CBMC_PBMC_deconvolution.Rdata"))
```


Load genotyping PCs and subset out REEGLE PCs (genotyping data contains PCs for a few thousand participants).
```{r genotypingPCS}
genotyping <- read.csv(file=here("input_data", "genotyping", "CHILD_subjects_Qced_first10PCS_Duan.csv"))
...
#prenatal  data
table1(~ no2_preg_entire + sex + prenatal_maternal_smoking +
         maternal_education_length + pss_6mos + gestational_days +         
         cbmc.CD4T + cbmc.NK + cbmc.Bcell + cbmc.Mono + cbmc.CD8T + cbmc.nRBC,
       data=all_vars[all_vars$prenatal_cc & all_vars$no2_preg_entire != 0 , ], 
       render.continuous = my.render.cont,
       render.categorical = my.render.cat)

# quantiles of air pollution
 quantile(all_vars[all_vars$prenatal_cc & all_vars$no2_preg_entire != 0, "no2_preg_entire"])


#age one data
table1(~ no2_preg_entire + sex + maternal_smoking_y1 +
         prenatal_maternal_smoking + maternal_education_length + pss_6mos + 
         pbmc.CD4T + pbmc.NK + pbmc.Bcell + pbmc.Mono + pbmc.CD8T,
       data=all_vars[all_vars$ageone_cc & all_vars$no2_preg_entire != 0, ], 
       render.continuous = my.render.cont,
       render.categorical = my.render.cat)

 quantile(all_vars[all_vars$ageone_cc & all_vars$no2_preg_entire != 0, "no2_preg_entire"], na.rm=T)


#year one data
table1(~ no2_y1_entire + sex + maternal_smoking_y1 +
         maternal_education_length + pss_6mos +         
         pbmc.CD4T + pbmc.NK + pbmc.Bcell + pbmc.Mono + pbmc.CD8T,
       data=all_vars[all_vars$yearone_cc, ], 
       render.continuous = my.render.cont,
       render.categorical = my.render.cat)

quantile(all_vars[all_vars$yearone_cc , "no2_y1_entire"], na.rm=T)
