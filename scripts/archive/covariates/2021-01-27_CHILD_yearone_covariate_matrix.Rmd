...
```{r covariates}
load(here("output_data", "covariates", "2021-09-24_CHILD_covariates_na_with_means.Rdata"))
```


Read in cell type estimates and PCs
...
load(here("output_data", "preprocessed_data", "2021-09_25_CHILD_preprocessed_betas_pdata_annotation.Rdata"))

#subset pData into PBMCs (year 1)
y1_pdata <- subset(pdata_nodups, Tissue == "P")
dim(y1_pdata)

#subset pData into cbmcs (birth) - this group has one less participant
...
Convert deconvolution estimates and cell type PCsto dataframes.
...
covariates_nameans_y1 <- covariates_nameans[as.character(covariates_nameans$sampleid) %in% as.character(y1_pdata$Sample_Label),]
dim(covariates_nameans_y1) #145 113

# combine covariate master data and cell type PCS
# check if they are identical 
identical(as.character(covariates_nameans_y1$sampleid), as.character(y1_pdata$Sample_Label)) # TRUE
identical(rownames(y1_pdata), rownames(y1_pcs)) # TRUE

covariates_nameans_y1 <- cbind(covariates_nameans_y1, y1_pcs)

# combine covariate data with genotyping data
rownames(covariates_nameans_y1)  <- covariates_nameans_y1$sampleid
covariates_nameans_y1 <- merge(covariates_nameans_y1, genosub[,2:4], by = "row.names", all.x = TRUE)
```
...
Load correlation heat map function
```{r cor_function}
source(file=here("scripts", "2021-08-30_corr_mat_funs.R"))
```


Get pvalues for correlations on a subset of covariates of interest.
```{r get-pvals}
covariates_nameans_y1_sub <- covariates_nameans_y1 %>%
  subset(complete.cases(no2_preg_entire, sex, gestational_days,
                        maternal_smoking_y1, prenatal_maternal_smoking,
                        maternal_education_length,pss_6mos, pss_18wk,
                        PC1, PC2, PC3,
                        genotyping.PC1, genotyping.PC2, genotyping.PC3)) %>%
  subset(no2_preg_entire > 0) %>%
  subset(sampleid %in% cb_pdata$Sample_Label)

dim(covariates_nameans_y1_sub) # 92

#pvals calcs
covariates_nameans_y1_sub_pval <- pval.mat(covariates_nameans_y1_sub %>%
                                             select(no2_preg_entire, sex, 
                                                    maternal_smoking_y1, 
                                                    maternal_education_length,
                                                    pss_6mos, PC1, PC2, PC3,
                                                    genotyping.PC1,
                                                    genotyping.PC2, 
                                                    genotyping.PC3))

#correlation calcs
covariates_nameans_y1_sub_corr <- corr.mat(covariates_nameans_y1_sub %>%
                                             select(no2_preg_entire, sex, 
                                                    maternal_smoking_y1, 
                                                    maternal_education_length,
                                                    pss_6mos, PC1, PC2, PC3,
...
corrmat <- corr.plot(corr = covariates_nameans_y1_sub_corr,
                     pval = covariates_nameans_y1_sub_pval,
                     colors = c("#6D9EC1", "white", "#E46726"), 
                     type = "lower",
                     show.diag =F,
                     x.axis.text.angle = 90) + 
  scale_y_discrete(position = "right",
                   labels = c("Prenatal NO2", "Biological sex", 
                              "Maternal smoking year one", 
                              "Maternal education length",
                              "Maternal stress 6 months", 
                              "PBMC PC1", "PBMC PC2", "PBMC PC3",
                              "Genotyping PC1", "Genotyping PC2")) +
  scale_x_discrete(labels = c("Biological sex", "Maternal smoking year one",
                              "Maternal education length",
                              "Maternal stress 6 months", 
                              "PBMC PC1", "PBMC PC2", "PBMC PC3",
                              "Genotyping PC1", "Genotyping PC2",
