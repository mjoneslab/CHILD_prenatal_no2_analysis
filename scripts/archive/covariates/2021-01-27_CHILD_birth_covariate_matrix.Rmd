...
load(file = here("output_data", "preprocessed_data", "2021-01-14_CHILD_preprocessed_betas_pdata_annotation.Rdata"))
...
Subset out children who have a prenatal no2 exposure of 0
```{r no2_zero}
covariates_nameans_geno_ctpc_tech_cc <- covariates_nameans_geno_ctpc_tech_cc[covariates_nameans_geno_ctpc_tech_cc$no2_preg_entire != 0, ]

dim(covariates_nameans_geno_ctpc_tech_cc)
```

Add in surrogate variables for batch effects and other unaccounted biological influences.
```{r svs}
load(file = here("output_data", "linear_regression", "2021-06-06_prenatal_svs.RData"))

svs <- svobj_all$u
# subset for participants with svs
...
corrplot(covariates_nameans_geno_ctpc_tech_cc_sub_corr,
  ...
  scale_y_discrete(labels = c("Prenatal NO2", "Maternal smoking", "Sex", 
                              "Gestational age", "Maternal stress", 
                              "Maternal education length", 
...
ggsave(here("figures", "covariates", "2021-06-06_CHILD_prenatal95_corrplot.png"),
       device="png",
       height= 8,
       width= 15 ,
       units="cm")
