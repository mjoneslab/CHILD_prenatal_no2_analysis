...
corr.plot <- function(corr,
                      ...
                      label.sig.pval = TRUE,
                      label.insig.pval = FALSE,
                     # pval.label.method = c("numeric", "symbol"),
                      pval.label.colour = "black",
                      pval.label.size = 4, 
                      sig.level = 0.05, 
                      pval.symbol.shape = 4, 
                      axis.text.size = 12, 
  ...
  #pval.label.method <- match.arg(pval.label.method)
  ...
  label <- round(x = corr[, "pvalue"], digits = digits)
  
  if (!is.null(pval) & !label.insig.pval) {
    # get which ones are not significant
    sig <- corr$pvalue < sig.level
    
    p <- p + geom_text(data = corr[sig,],
                       mapping = aes_string(x = "Var1", y = "Var2"), 
                       label = label[sig], size = pval.label.size)
    
  } else if (!is.null(pval) & label.insig.pval) {
    p <- p + geom_text(data = corr,
                       mapping = aes_string(x = "Var1", y = "Var2"), 
                       label = label, size = pval.label.size)
  }
...
# svcorr <- p + geom_text(data = corr,
#                         mapping = aes_string(x = "Var1", y = "Var2"), 
#                         label = label, size = 1.25) + 
#   theme_minimal() + 
